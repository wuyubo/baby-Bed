...
void camera_refresh(void)
{
	...
	for(j=0;j<76800;j++)
	{
		HT_GPIOB->DIRCR=0X00000000;		//设置所有B端口为上拉输入  	
		HT_GPIOB->PUR=0X0000FFFF;	
		HT_GPIOB->INER=0X0000FFFF;	
		OV7670_RCK(0); 
		...
		color|=OV7670_DATA;		//读数据		  
		OV7670_RCK(1); 
		HT_GPIOB->DIRCR=0X0000FFFF;		//设置所有B端口为推挽输出  
		buf[j%10] = color;
		if(j%10 == 0 && j>0)
		{
			Wifi_Send((u8*)buf, 20);
