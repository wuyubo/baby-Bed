...
static  void  OS_SchedNew (void)
{
#if OS_LOWEST_PRIO <= 63                /* See if we support up to 64 tasks   根据任务数是否大于64来采取不同的计算方案 */
    INT8U   y;


    y             = OSUnMapTbl[OSRdyGrp]; /*OSUnMapTbl可以理解为一个解码表*/
    OSPrioHighRdy = (INT8U)((y << 3) + OSUnMapTbl[OSRdyTbl[y]]); /* OSReadyTbl的大小为[OS_LOWEST_PRIO/8+1]*/
    ...
    if ((OSRdyGrp & 0xFFu) != 0) {
        ...
        y = OSUnMapTbl[(OSRdyGrp >> 8) & 0xFFu] + 8;
    }
    ptbl = &OSRdyTbl[y];
    if ((*ptbl & 0xFFu) != 0u) {
        OSPrioHighRdy = (INT8U)((y << 4u) + OSUnMapTbl[(*ptbl & 0xFFu)]);
    } else {
        OSPrioHighRdy = (INT8U)((y << 4u) + OSUnMapTbl[(*ptbl >> 8) & 0xFFu] + 8u);
    }
#endif
}
